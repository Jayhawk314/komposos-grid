...
from domains.grid.run_coverage_audit import _post_ia_pct
...
# ─────────────────────────────────────────────────────────────────────────────
# PAGE: Post-IA Robustness Check
# ─────────────────────────────────────────────────────────────────────────────

elif selection == "🔬 Post-IA Robustness Check":
    _stitch_badge("Post-IA Finding — Robustness Checks")
    st.title("🔬 Post-IA Robustness Check")
    _provenance_badge(
        "derived",
        "Both panels on this page are this project's own computation from the LBNL "
        "Queued Up workbook — extensions of a published method to slices LBNL does not "
        "publish. See Regional Queue Study for the underlying post-IA figures themselves.",
    )
    st.markdown(
        "The Regional Queue Study page reports MISO at **34.9%** post-IA completion "
        "against ERCOT at **79.7%** — a signed interconnection agreement means very "
        "different things by region. Two follow-up questions a careful reader should "
        "ask: **can that comparison even be made everywhere**, and **does it survive "
        "when the youngest IA cohorts are removed at common age cutoffs**? This page "
        "answers both, generated by "
        "`run_post_ia_horizon.py` and `run_coverage_audit.py`."
    )

    horizon_path = REPORTS_DIR / "post_ia_horizon" / "post_ia_horizon.json"
    coverage_path = REPORTS_DIR / "coverage_audit" / "coverage_audit.json"

    tab_horizon, tab_coverage, tab_export = st.tabs(
        ["⏳ Minimum-Follow-Up Check", "🧭 Coverage Audit", "⬇️ Export"]
    )

    with tab_horizon:
        if horizon_path.exists():
            hz = json.loads(horizon_path.read_text(encoding="utf-8"))
            hz_prov = hz["provenance"]

            st.subheader("Does the MISO/ERCOT gap survive minimum-follow-up cutoffs?")
            st.markdown(
                "A raw pooled rate mixes signed projects of very different ages — a "
                "region whose IAs are on average younger has had less time for either "
                "outcome to occur. This restricts the comparison to signings old enough, "
                f"as of the **{hz_prov['vintage_cutoff']}** data vintage, to have had a "
                "fair chance to resolve by 2, 3, and 5 years after execution. This "
                "applies the same age floor to both regions; it does not match their "
                "remaining age distributions."
            )

            gap_rows = hz["miso_ercot_gap_by_horizon"]
            g_cols = st.columns(len(gap_rows))
            for col, g in zip(g_cols, gap_rows):
                gap_text = f"{g['gap'] * 100:.1f} pp" if g["gap"] is not None else "—"
                rates_text = (
                    f"MISO {g['miso_rate']:.0%} · ERCOT {g['ercot_rate']:.0%}"
                    if g["miso_rate"] is not None and g["ercot_rate"] is not None
                    else "—"
                )
                col.metric(g["label"].title(), gap_text, rates_text, delta_color="off")

            gap_chart_df = pd.DataFrame(
                {
                    "Cohort": [g["label"] for g in gap_rows],
                    "MISO": [((g["miso_rate"] or 0) * 100) for g in gap_rows],
                    "ERCOT": [((g["ercot_rate"] or 0) * 100) for g in gap_rows],
                }
            ).set_index("Cohort")
            st.line_chart(gap_chart_df)
            st.caption(
                "Post-IA completion rate (%) at each minimum-follow-up cutoff. The "
                "nearly unchanged gap shows that simply removing the youngest signings "
                "does not explain the raw difference; this is not a matched-age result."
            )

            st.divider()
            st.subheader("Why minimum follow-up matters here: cohort age by region")
            age_rows = [
                {
                    "Region": REGION_DISPLAY.get(rr["region"], rr["region"].upper()),
                    "Median Months Since IA": rr["cohort_age_months"]["median_months"],
                }
                for rr in hz["regions"]
                if rr["cohort_age_months"]["n"]
            ]
            if age_rows:
                age_df = pd.DataFrame(age_rows).sort_values(
                    "Median Months Since IA", ascending=False
                )
                st.bar_chart(age_df.set_index("Region"))
            st.caption(
                "MISO's signed cohort carries a long historical tail back to 2001–2002; "
                "ERCOT's is dominated by the 2018–2025 surge. The checks above show the "
                "youngest cohorts are not driving the gap. They do not fully control "
                "for age, because the remaining regional age distributions still differ."
            )

            st.divider()
            st.subheader("Full breakdown by region")
            region_labels = [
                REGION_DISPLAY.get(rr["region"], rr["region"].upper()) for rr in hz["regions"]
            ]
            sel_label = st.selectbox(
                "Select region", region_labels, key="horizon_region_select"
            )
            rr = next(
                r for r in hz["regions"]
                if REGION_DISPLAY.get(r["region"], r["region"].upper()) == sel_label
            )
            raw = rr["raw_post_ia"]
            raw_label = (
                f"{raw['rate']:.1%}" if raw["signed_decided"] else "not computable (n=0)"
            )
            st.markdown(
                f"**Raw post-IA rate:** {raw_label} "
                f"({raw['built_after_signing']:,}/{raw['signed_decided']:,})"
            )
            hdf = pd.DataFrame(rr["horizons"])
            hdf["Horizon"] = hdf["horizon_months"].apply(lambda m: f"{m // 12}y")
            hdf["Rate"] = hdf["rate"].apply(
                lambda v: f"{v:.1%}" if v is not None else "— (n=0)"
            )
            hdf = hdf.rename(columns={
                "n_mature": "Mature", "operational": "Operational",
                "withdrawn": "Withdrawn",
                "censored_active_or_suspended": "Censored",
                "decided": "Decided", "classification": "Classification",
            })
            st.dataframe(
                hdf[[
                    "Horizon", "Mature", "Operational", "Withdrawn", "Censored",
                    "Decided", "Rate", "Classification",
                ]],
                use_container_width=True, hide_index=True,
            )
            st.caption(
                "Censored = still active/suspended despite being old enough to expect "
                "resolution — reported, never dropped or counted as a failure."
            )

            with st.expander("Method note"):
                st.markdown(hz_prov["method_note"])
        else:
            st.error(
                "Minimum-follow-up JSON not found. Run "
                "`python -m domains.grid.run_post_ia_horizon` first."
            )

    with tab_coverage:
        if coverage_path.exists():
            cov = json.loads(coverage_path.read_text(encoding="utf-8"))
            cov_prov = cov["provenance"]

            st.subheader("Can the post-IA comparison even be made everywhere?")
            st.markdown(cov_prov["post_ia_observability_note"])

            def _class_color(v):
                return {
                    "complete": "color: #22c55e; font-weight: 700;",
                    "partial": "color: #f59e0b; font-weight: 600;",
                    "absent": "color: #f87171; font-weight: 700;",
                }.get(v, "")

            obs_df = pd.DataFrame([
                {
                    "Region": REGION_DISPLAY.get(row["region"], row["region"].upper()),
                    "Withdrawn w/ ia_date": row["with_ia_date"],
                    "Total Withdrawn": row["total_withdrawn"],
                    "%": _post_ia_pct(row),
                    "Classification": row["classification"],
                }
                for row in cov["national"]["post_ia_observability_by_region"]
            ])
            st.dataframe(
                obs_df.style.map(
                    _class_color, subset=["Classification"]
                ),
                use_container_width=True, hide_index=True,
            )

            st.divider()
            st.subheader("Notable finding: wd_date coverage varies sharply by region")
            st.markdown(
                "`wd_date` (withdrawal date) is not used by any pipeline in this repo "
                "today, but it is exactly what a precise time-to-event analysis would "
                "need to know *when* a withdrawn project's clock stopped. Coverage "
                "within each region's withdrawn cohort, worst first:"
            )
            wd_df = pd.DataFrame([
                {
                    "Region": REGION_DISPLAY.get(r["region"], r["region"].upper()),
                    "wd_date Coverage (%)": (r["pct"] or 0) * 100,
                }
                for r in cov["national"]["wd_date_coverage_by_region_withdrawn"]
            ]).set_index("Region")
            st.bar_chart(wd_df)
            st.caption(
                "West (12%) and the Southeast (35%) sit far below MISO/CAISO/PJM/ISO-NE "
                "(94–99%) — a real data-harmonization gap, not a rounding difference."
            )

            with st.expander("Field definitions (mw and hybrid capacity)"):
                for f, desc in cov_prov["populated_definitions"].items():
                    st.markdown(f"- **{f}** — {desc}")
        else:
            st.error(
                "Coverage audit JSON not found. Run "
                "`python -m domains.grid.run_coverage_audit` first."
            )

    with tab_export:
        st.subheader("Export")
        for label, path, mime in [
            ("📥 Download Minimum-Follow-Up Check (Markdown)",
             REPORTS_DIR / "post_ia_horizon" / "post_ia_horizon.md", "text/markdown"),
            ("📥 Download Minimum-Follow-Up Check (JSON)",
             REPORTS_DIR / "post_ia_horizon" / "post_ia_horizon.json", "application/json"),
            ("📥 Download Coverage Audit (Markdown)",
             REPORTS_DIR / "coverage_audit" / "coverage_audit.md", "text/markdown"),
            ("📥 Download Coverage Audit (JSON)",
             REPORTS_DIR / "coverage_audit" / "coverage_audit.json", "application/json"),
        ]:
            if path.exists():
                st.download_button(label, path.read_bytes(), path.name, mime)
            else:
                st.caption(f"`{path.name}` not yet generated.")
