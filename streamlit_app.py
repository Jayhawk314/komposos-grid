...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    st.markdown(
        ...
        "once cohort age is controlled for**? This page answers both, generated by "
    ...
    tab_horizon, tab_coverage, tab_export = st.tabs(
        ["⏳ Time-Since-IA Control", "🧭 Coverage Audit", "⬇️ Export"]
    )
    ...
    with tab_horizon:
        if horizon_path.exists():
            ...
            st.subheader("Does the MISO/ERCOT gap survive controlling for cohort age?")
            st.markdown(
                ...
                "fair chance to resolve by 2, 3, and 5 years after execution."
            )
            ...
            st.caption(
                "Post-IA completion rate (%) at each cohort-age cut. Converging lines "
                "would mean the raw gap is partly a maturity artifact; a flat, parallel "
                "pair means it isn't — which is what these three checks show."
            )

            st.divider()
            st.subheader("Why maturity control matters here: cohort age by region")
            age_rows = [
                {
            ...
            st.caption(
                ...
                "ERCOT's is dominated by the 2018–2025 surge. If recency bias were "
                "driving the raw gap, matching cohort age should have moved the numbers "
                "above — it didn't."
            )
            ...
            st.error(
                "Horizon control JSON not found. Run "
    ...
    with tab_export:
        st.subheader("Export")
        for label, path, mime in [
            ("📥 Download Horizon Control (Markdown)",
             REPORTS_DIR / "post_ia_horizon" / "post_ia_horizon.md", "text/markdown"),
            ("📥 Download Horizon Control (JSON)",
